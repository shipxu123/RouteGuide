...
def prepare_data(ds):
    ...
    ds.read_data_labeled_net("../bench_dif_nets")
    ds.read_data_test("")
    ds.data_augment()
...
def train_labeled_vae(ds):
    ...
    re_flags.DEFINE_integer("epochs", 300, "number of epoch")
    ...
    vae.encoder.load_state_dict(state_dict)
    vae = vae.cuda()
    ...
    # util.draw_five_generated_samples(64, test.cpu(), label, sample[0])

    vae.save_encoder("weights/vae_gen_dif_fix_32_test/encoder", prefix="gen/encoder")
    vae.save_decoder("weights/vae_gen_dif_fix_32_test/decoder", prefix="gen/decoder")
